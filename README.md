...
```mermaid
flowchart TD
    entry["cron(每小時)/ scrape 子指令"] --> fetch["OPENTIX search API<br/>(offset 分頁全量掃描,頁間 500ms)"]
    fetch --> norm["正規化 Event<br/>(含場館、演出時間、開賣時間)"]
    norm --> redis["Redis SET NX 快篩"]
    redis -->|新的才往下| pg["Postgres INSERT .. ON CONFLICT DO NOTHING<br/>(去重最終判準)"]
    redis -.->|Redis 故障時降級| pg
    pg -->|insert 成功才通知| match["關鍵字比對"]
    match --> discord["Discord Webhook<br/>(最多嘗試 3 次,失敗不中斷)"]
```
...
- **韌性**:抓取與推播各最多嘗試 3 次(指數退避)、單筆壞資料跳過不全黑、graceful shutdown 會等進行中的抓取收尾
...
沒有 Discord webhook 也能跑:`go run ./cmd/app serve -no-notify`(無推播模式,照常抓取入庫)。
...
docker compose run --rm app scrape -config /config.yaml -no-notify   # 初次 seed:只入庫不推播
docker compose up -d
